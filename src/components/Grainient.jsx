...
void mainImage(out vec4 o, vec2 C){
  ...
  vec3 layer1=mix(col3,col2,S(edge0,edge1,blendX));
  vec3 layer2=mix(col2,col1,S(edge0,edge1,blendX));
  vec3 col=mix(layer1,layer2,S(vertical0,vertical1,tuv.y));
...
function Grainient({
  ...
  useEffect(() => {
    ...
    let pageVisible = !document.hidden && !prefersReducedMotion;
    ...
    const tryStart = () => {
      if (inViewport && pageVisible && raf === 0) {
        raf = window.requestAnimationFrame(renderLoop);
      }
    };
    ...
    const handleVisibility = () => {
      pageVisible = !document.hidden && !prefersReducedMotion;
      if (pageVisible) {
        tryStart();
      } else {
        tryStop();
  ...
  useEffect(() => {
    const container = containerRef.current;
    const context = container ? contextMap.get(container) : null;
    if (!context) return;
